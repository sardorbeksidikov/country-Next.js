...
const ThemeContextProvider: FC<{children:ReactNode}> = ({children}) => {
   ...
   const [themeContextState,setThemeContextState] = useState({mode:THEME_MODE.LIGHT_MODE})

   useEffect(() => {
     const defaultTheme = localStorage.getItem('theme') 
     const userPreferDarkTheme = window.matchMedia('(prefers-color-scheme: dark)')
      if(defaultTheme){
   ...
   function toggleTheme(){
      setThemeContextState(prevTheme => {
         return {...prevTheme,mode:prevTheme.mode === THEME_MODE.DARK_MODE ? THEME_MODE.LIGHT_MODE : THEME_MODE.DARK_MODE}
      })
